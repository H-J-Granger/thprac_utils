...
using glyph_interval = std::pair<wchar_t, wchar_t>;
...
struct game_t {
	...
	static set<glyph_interval> glyph_range_zh;
	static set<glyph_interval> glyph_range_en;
	static set<glyph_interval> glyph_range_ja;

	game_t() = default;

	static set<glyph_interval>& get_glyph_range(Language language) {
		switch (language) {
			case Language::Chinese:
...
set<glyph_interval> game_t::glyph_range_zh;
set<glyph_interval> game_t::glyph_range_en;
set<glyph_interval> game_t::glyph_range_ja;
...
void AddGlyphRange(loc_str_t& str) {
	...
	for (auto c : u16str) game_t::glyph_range_zh.emplace(c, c);

	u16str = conv.from_bytes(str.en_str);
	for (auto c : u16str) game_t::glyph_range_en.emplace(c, c);

	u16str = conv.from_bytes(str.ja_str);
	for (auto c : u16str) game_t::glyph_range_ja.emplace(c, c);
}

void MergeGlyphRange() {
	for (auto glyph_range_ptr : {&game_t::glyph_range_zh, &game_t::glyph_range_en, 
                                 &game_t::glyph_range_ja}) {
		auto &glyph_range = *glyph_range_ptr;
		wchar_t begin = 0, end = 0;
		auto range2 = glyph_range;
		for (auto interval : range2) {
			auto code = interval.first;
			if (code <= 0xFF) { continue; }
			if (code == end + 1) {
				if (begin == end) {
					glyph_range.erase({begin, begin});
				}
				end++;
				glyph_range.erase({code, code});
			} else {
				if (begin < end) {
					glyph_range.emplace(begin, end);
				}
				begin = end = code;
			}
		}
...
void write_glyph_range_declaration(
	string& output,
	set<glyph_interval>& glyph_range,
	Language language
) {
	// Filter and count the glyph ranges
	size_t glyph_range_filtered_size = 0;
	for (auto interval : glyph_range) {
		auto begin = interval.first;
		if (begin > 0xFF) glyph_range_filtered_size += 2;
	}
...
void write_glyph_range_definition(
	...
	for (auto interval : glyph_range) {
		auto begin = interval.first, end = interval.second;
		// It's guarenteed that the interval won't cross 0xFF
		if (begin <= 0xFF) continue;  
		sprintf_append(definition_body, "    " "%#x, %#x," ENDL, begin, end);
